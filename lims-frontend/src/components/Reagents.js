...
import useInfiniteReagents from './hooks/useInfiniteReagents'; 
...
// Local PrinterIcon (in case it's missing from ./Icons)
...
const COLUMN_WIDTHS = {
  expandIcon: '32px',
  actions: '260px', // Increased for Print button
  gridColumns: '2fr 1fr 80px 1fr 1fr 1fr 100px'
};

// ==================== CHELATE Accordion Styles ====================
...
const ReagentAccordionItem = ({ 
  ...
  // Handler for Print button on reagent level
  const handlePrintClick = async (e) => {
    e.stopPropagation();
    // Load batches if not loaded yet
    if (batches.length === 0) {
      setLoadingBatches(true);
      try {
        const data = await api.getReagentBatches(reagent.id);
    ...
    <div style={accordionStyles.container}>
      ...
      >
        ...
        <div style={accordionStyles.reagentInfo}>
          ...
          <div>
            <Badge variant={reagent.status === 'active' ? 'success' : reagent.status === 'inactive' ? 'warning' : 'danger'}>
      ...
      {/* Print Sticker Modal - available at reagent level */}
      {showPrintModal && (
        <PrintStickerModal 
...
const ReagentsEnhanced = ({ user }) => {
  ...
  // ИСПРАВЛЕНИЕ: Формируем объект фильтров, исключая пустые строки.
  // Если передать status: "", сервер будет искать точное совпадение с пустой строкой, что вернет 0 результатов.
  const activeFilters = useMemo(() => {
    ...
    return filters;
  }, [searchTerm, statusFilter, manufacturerFilter, stockFilter, casNumberFilter, sortBy, sortOrder]);

  // Hook Initialization with cleaned filters
  const { 
    reagents, 
    loading, 
    loadingMore,
    ...
    refresh, 
    removeReagent 
  } = useInfiniteReagents(activeFilters);
  ...
  // ИСПРАВЛЕНИЕ: Восстановлено динамическое обновление списка производителей
  useEffect(() => {
    if (reagents.length > 0) {
      const uniqueManufacturers = [...new Set(reagents.map(r => r.manufacturer).filter(Boolean))].sort();
      // Объединяем с дефолтными, чтобы не потерять их, если текущий список фильтрован
      setManufacturers(prev => [...new Set([...prev, ...uniqueManufacturers])].sort());
    }
  }, [reagents]);

  // Загрузка allBatches для Import Modal
  useEffect(() => {
    const loadReferenceData = async () => {
      try {
        ...
        console.error("Failed to load reference data (batches):", e);
      }
    };
    loadReferenceData();
  }, []);
      ...
      case 'delete':
        if (window.confirm(`Delete reagent "${reagent.name}"? This will also delete all associated batches.`)) {
          try {
            ...
            removeReagent(reagent.id); // Optimistic update via hook
          } catch (err) {
  ...
  return (
    <div style={{ padding: '20px', paddingTop: '90px' }}>
      ...
      <div style={{ marginBottom: '15px', color: '#718096', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span>
          Showing <strong style={{ color: '#1a365d' }}>{reagents.length}</strong> of <strong style={{ color: '#1a365d' }}>{total}</strong> reagents
        </span>
      </div>
        ...
        {reagents.length === 0 && !loading ? (
          <div style={{ textAlign: 'center', padding: '60px 40px', color: '#718096', backgroundColor: '#fff', borderRadius: '0 0 12px 12px', border: '1px solid #e2e8f0', borderTop: 'none' }}>
            <div style={{
              ...
              height: '80px',
              margin: '0 auto 20px',
              background: 'linear-gradient(135deg, rgba(49, 130, 206, 0.1) 0%, rgba(56, 161, 105, 0.1) 100%)',
              borderRadius: '50%',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center'
            ...
            <p style={{ margin: '0 0 20px 0', fontSize: '0.9rem' }}>
              {activeFiltersCount > 0 
                ...
                : "Get started by adding your first reagent."}
            </p>
          </div>
        ) : (
        ...
        {hasMore && !loading && (
          <div style={{ textAlign: 'center', marginTop: '20px' }}>
            <Button onClick={loadMore} disabled={loadingMore}>
              {loadingMore ? 'Loading more...' : 'Load More Reagents'}
            </Button>
          </div>
        )}
      </div>
